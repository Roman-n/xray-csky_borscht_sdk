...
*/
	...
	struct CHelper4 {
		IC	static void load_data(T &data, M &stream, const P &p)
		{
			CHelper<T,object_type_traits::is_pointer<T>::value>::load_data(data,stream,p);
		}
	};
	...
	IC	static void load_data(T &data, M &stream, const P &p)
	{
		CHelper4<T,object_type_traits::is_stl_container<T>::value>::load_data(data,stream,p);
	}
};
